...
const Logo = styled.div`
  font-size: 1.6rem;
  font-weight: 700;
  color: ${({ theme }) => theme?.primary || "#6a4cff"};
  cursor: pointer;
  display: flex;
  align-items: center;
  gap: 8px;
...
const Button = styled(motion.button)`
  ...
  ${({ $type, theme }) =>
    $type === "primary"
      ? `
    background: ${theme?.primary || "#6a4cff"};
    color: white;
    &:hover { background: #5a3fee; }
  `
      : `
    background: transparent;
    border-color: ${theme?.primary || "#6a4cff"};
    color: ${theme?.primary || "#6a4cff"};
    &:hover {
      background: ${theme?.primary || "#6a4cff"};
      color: white;
    }
    ...
    <HeaderContainer theme={theme}>
      ...
      <Logo theme={theme}>
        {typeof logo === "object" && logo.src ? (
          <img src={logo.src} alt={logo.text || "Logo"} />
        ) : (
          logo
        )}
      </Logo>
